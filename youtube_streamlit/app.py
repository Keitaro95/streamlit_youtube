...
def video_search(youtube, q='自動化', max_results=50):
    ...
    return df_video
# df_videosに動画id, チャンネルidが返ってきます
# 次、取得したチャンネルidから、そのチャンネルの購読者数をとってきます

# 引数にdf_videoと閾値 df_videoは検索した結果のdf 閾値は登録者数の制限値
def get_results(df_video, threshold=50000):
    channel_ids = df_video['channel_id'].unique().tolist()

    subscriber_list = youtube.channels().list(
        id=','.join(channel_ids), #id には　取得するリソースの YouTube チャンネル ID のカンマ区切りのリストを指定
        part='statistics',
        fields='items(id,statistics(subscriberCount))' #id, statisticsの中のsubscriberCountを指定してそれだけをフィルタ
    ).execute()
...
if st.button('ビデオ表示'):
    if len(video_id) > 0:
        try:
            video_field.video(url) #これによりyoutube動画表示
        except:
            st.error(
                """
                **おっと！何かエラーが起きているようです。** :(
